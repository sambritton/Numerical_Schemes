...
uexact_x = @(x,t)((exp(-(pi)^2*t))*pi*cos(pi*x));
uexact_t = @(x,t)(-(pi)^2*((exp(-(pi)^2*t)))*sin(pi*x));
uexact_xx = @(x,t)(-(pi)^2*(exp((-(pi)^2*t)))*sin(pi*x));
f = @(x,t) 0;
...
for k = 1:iterations
    ...
    for n = 1:nodes_t-1
        ...
        % Backward Euler
        U(n+1,1:end)= (M+(dt*S))\(M*transpose(U(n,1:end)));
